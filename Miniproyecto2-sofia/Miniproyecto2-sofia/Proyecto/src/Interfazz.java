...
*/
    ...
    // Crear una instancia del panel de inserción
    InsertarPanel insertarCandidato = new InsertarPanel();

    // Mostrar el panel en una ventana emergente (JFrame)
    JFrame ventanaInsertar = new JFrame("Insertar Candidato");
    ventanaInsertar.add(insertarCandidato);
    ventanaInsertar.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);  // Cerrar solo la ventana emergente, no la aplicación principal
    ventanaInsertar.setSize(400, 300);
    ventanaInsertar.setVisible(true);

  
    insertarCandidato.getInsertButton().addActionListener(new ActionListener() {
        @Override
        public void actionPerformed(ActionEvent e) {
           

           
            JOptionPane.showMessageDialog(null, "Candidato añadido exitosamente.", "Éxito", JOptionPane.INFORMATION_MESSAGE);

            
            JButton volverButton = new JButton("Volver");
            insertarCandidato.add(volverButton);
